...
def main():
    ...
    # load files
    try:
        gt = np.loadtxt('/home/yunfan/work_space/master_thesis/rpg_trajectory_evaluation/results/cmp_tracking/desktop/dvspanotracking/desktop_dvspanotracking_dynamic_rotation/stamped_groundtruth.txt')
        gt_t = gt[:, 0]
        gt_vec = R.from_quat(gt[:, 4:8]).as_rotvec()

        pano = np.loadtxt(
            '/home/yunfan/work_space/master_thesis/rpg_trajectory_evaluation/results/cmp_tracking/desktop/dvspanotracking/desktop_dvspanotracking_dynamic_rotation/stamped_traj_estimate.txt')
        pano_t = pano[:, 0]
        pano_vec = R.from_quat(pano[:, 4:8]).as_rotvec()

        glo = np.loadtxt(
            '/home/yunfan/work_space/master_thesis/rpg_trajectory_evaluation/results/cmp_tracking/desktop/globallyaligned/desktop_globallyaligned_dynamic_rotation/stamped_traj_estimate.txt')
        glo_t = glo[:, 0]
        glo_vec = R.from_quat(glo[:, 4:8]).as_rotvec()

        mos = np.loadtxt(
            '/home/yunfan/work_space/master_thesis/rpg_trajectory_evaluation/results/cmp_tracking/desktop/mosaic/desktop_mosaic_dynamic_rotation/stamped_traj_estimate.txt')
        mos_t = mos[:, 0]
        mos_vec = R.from_quat(mos[:, 4:8]).as_rotvec()
    ...
    ax_x.plot(pano_t, pano_vec[:, 0], color='red',
              linestyle='solid', lw=2.5, label='RTPT')
    # ax_x.plot(gt_t,   gt_vec[:, 0], color='green',
    #           linestyle='solid', lw=2.5, label='ground truth')
    ax_x.plot(glo_t,  glo_vec[:, 0], color='blue',
              linestyle='solid', lw=2.5, label='CMaxGAE')
    ax_x.plot(mos_t,  mos_vec[:, 0], color='black',
              linestyle='solid', lw=2.5, label='SMT')
    # ax_x.plot(pose_dr_t, pose_dr_x,
    #           color='yellow', linestyle='solid', lw=2.5)
    ax_x.set_xlim(15, 25)
    ax_x.set_ylim(-0.5, 0.7)
    ax_x.set_title('x-axis', fontsize=20)
    ax_x.xaxis.set_major_locator(MaxNLocator(5))
    ax_x.yaxis.set_major_locator(MaxNLocator(3))
    ax_x.tick_params(axis='x', labelsize=15, direction='in')
    ax_x.tick_params(axis='y', labelsize=15, direction='in')
    ...
    ax_y.plot(mos_t,  mos_vec[:, 1], color='black', linestyle='solid', lw=2.5)
    # ax_y.plot(pose_dr_t, pose_dr_y,
    #           color='yellow', linestyle='solid', lw=2.5)
    ax_y.set_xlim(15, 25)
    ax_y.set_ylim(-0.8, 0.6)
    ax_y.set_title('y-axis', fontsize=20)
    ax_y.xaxis.set_major_locator(MaxNLocator(5))
    ax_y.yaxis.set_major_locator(MaxNLocator(3))
    ax_y.tick_params(axis='x', labelsize=15, direction='in')
    ax_y.tick_params(axis='y', labelsize=15, direction='in')
    ...
    ax_z.plot(mos_t,  mos_vec[:, 2], color='black', linestyle='solid', lw=2.5)
    # ax_z.plot(pose_dr_t, pose_dr_z,
    #           color='yellow', linestyle='solid', lw=2.5)
    ax_z.set_xlim(15, 25)
    ax_z.set_ylim(-1.5, 0.2)
    ax_z.set_title('z-axis', fontsize=20)
    ax_z.xaxis.set_major_locator(MaxNLocator(5))
    ax_z.yaxis.set_major_locator(MaxNLocator(3))
    ax_z.tick_params(axis='x', labelsize=15, direction='in')
    ax_z.tick_params(axis='y', labelsize=15, direction='in')
